...
def callback():
    ...
    try:
        ...
        # Check if email is allowed (all staff from allowed domains)
        if not is_email_allowed(email):
            return render_template_string('''
                ...
                <body style="font-family: sans-serif; max-width: 600px; margin: 50px auto; text-align: center;">
                    <h1>🚫 Access Denied</h1>
                    <p>Your email address ({{ email }}) is not from an authorized domain.</p>
                    <p>Only staff members can access Sadie.</p>
                </body>
                </html>
            ''', email=email)
