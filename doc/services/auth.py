...
from shared.auth.email_check import is_email_allowed_by_domain, is_admin_user
...
def is_email_allowed(email):
    """Check if email is allowed to access Doc (company domain only)."""
    return is_email_allowed_by_domain(email, config.allowed_domains)


def is_admin(email):
    """Check if email is an admin."""
    return is_admin_user(email, config.admin_emails)
