"""
Bootstrap Buz authentication using Playwright storage state.

This tool helps you authenticate to each Buz organization and save the session
to a storage state file. All Buz-interacting bots (Banji, Hugo, Ivy) use these
files to maintain authenticated sessions without needing passwords.
...
These files are used by:
    - Banji (quote automation)
    - Hugo (user management)
    - Ivy (inventory/pricing sync)
"""
...
# Shared secrets directory at project root
...
async def main(org_name: str = "default") -> None:
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            print(f"WARNING: Could not find user table!")
            print(f"  Current URL: {page.url}")
        ...
        try:
            ...
            print(f"\nTimeout waiting for console1 navigation")
            print(f"  Current URL: {page.url}")
        ...
        print("\n> All Buz bots (Banji, Hugo, Ivy) can now use this storage state")
        print("="*80 + "\n")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print("\nStorage states are saved to: .secrets/buz/")
        print("Used by: Banji, Hugo, Ivy")
        sys.exit(1)
